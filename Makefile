...
OUT_JSON := --out json=$(RESULTS_DIR)/$(TEST_NAME)-$(TIMESTAMP).json
SUMMARY_HTML := K6_SUMMARY_HTML=$(RESULTS_DIR)/$(TEST_NAME)-$(TIMESTAMP).html
...
smoke: results-dir ## Run smoke test (~30s, 1 VU) — fail fast on broken endpoints
	$(SUMMARY_HTML) $(K6) run -e ENV=$(ENV) $(OUT_JSON) tests/smoke/api-smoke.js

load: TEST_NAME=load
load: results-dir ## Run load test (~8 min, ramping to peak)
	$(SUMMARY_HTML) $(K6) run -e ENV=$(ENV) $(OUT_JSON) tests/load/browse-ratings.js
...
load-lifecycle: results-dir ## Run pizza-lifecycle load test (~8 min, login + generate + rate)
	$(SUMMARY_HTML) $(K6) run -e ENV=$(ENV) $(OUT_JSON) tests/load/pizza-lifecycle.js

stress: TEST_NAME=stress
stress: results-dir ## Run stress test (~15 min, push past capacity)
	$(SUMMARY_HTML) $(K6) run -e ENV=$(ENV) $(OUT_JSON) tests/stress/auth-stress.js

spike: TEST_NAME=spike
spike: results-dir ## Run spike test (~3 min, sudden 20x burst)
	$(SUMMARY_HTML) $(K6) run -e ENV=$(ENV) $(OUT_JSON) tests/spike/public-spike.js

soak: TEST_NAME=soak
soak: results-dir ## Run soak test (~2h, steady load — memory leak detection)
	$(SUMMARY_HTML) $(K6) run -e ENV=$(ENV) $(OUT_JSON) tests/soak/steady-state.js

breakpoint: TEST_NAME=breakpoint
breakpoint: results-dir ## Run breakpoint test (slow ramp, find capacity knee)
	$(SUMMARY_HTML) $(K6) run -e ENV=$(ENV) $(OUT_JSON) tests/breakpoint/capacity.js

e2e: TEST_NAME=e2e
e2e: results-dir ## Run full end-to-end user journey
	$(SUMMARY_HTML) $(K6) run -e ENV=$(ENV) $(OUT_JSON) scenarios/e2e-user-journey.js
